...
// A new card graded "good" comes back tomorrow, which is the behaviour asked
// for; thereafter intervals grow by the card's ease factor. "Again" both resets
// the interval and requeues the card inside the current session, so a word you
// blanked on gets tested twice before you leave.
...
export function schedule(srs, grade, now = new Date()) {
  const next = { ...srs };
  ...
  if (srs.state === 'new' || srs.interval === 0) {
    next.interval = grade === GRADE.EASY ? 3 : grade === GRADE.HARD ? 1 : 1;
...
export function previewIntervals(srs, now = new Date()) {
  return [0, 1, 2, 3].map((g) => {
    const s = schedule(srs, g, now);
